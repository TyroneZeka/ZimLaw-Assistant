...
def process_subsection(subsec):
    """Process a subsection and its nested elements (handles akn-paragraph)"""
    content_parts = []

    # Get the subsection number (e.g., (1), (2))
    num_tag = subsec.find('span', class_='akn-num')
    if num_tag:
        num_text = num_tag.get_text(strip=True)
        content_parts.append(num_text)

    # Get any introductory text
    ...
    # Process all akn-paragraph blocks within this subsection
    paragraph_blocks = subsec.find_all('section', class_='akn-paragraph')
    for para_block in paragraph_blocks:
        para_content = process_paragraph_block(para_block)
        if para_content:
            content_parts.append(para_content)

    # Also check for any direct akn-content (fallback for simpler subsections)
    direct_content = subsec.find('span', class_='akn-content')
    if direct_content and not paragraph_blocks:  # Only use this if no paragraphs were found
        text = extract_content_block(direct_content)
        if text:
...
def process_paragraph_block(para_block):
    """Process an akn-paragraph block"""
    parts = []
    # Get the paragraph letter (a), (b), etc.
    num_tag = para_block.find('span', class_='akn-num')
    if num_tag:
        parts.append(num_tag.get_text(strip=True))
    # Get the paragraph content
    content_span = para_block.find('span', class_='akn-content')
    if content_span:
        text = extract_content_block(content_span)
        if text:
            parts.append(text)
    return " ".join(parts)
