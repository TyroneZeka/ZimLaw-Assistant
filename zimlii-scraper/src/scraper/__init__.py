...
def is_repealed_section(section_div):
    ...
    return "[section repealed by" in text.lower()
...
def scrape_zimlii_act(url):
    ...
    for div in all_section_divs:
        if is_repealed_section(div):
            print(f"🗑️ Skipping repealed section")
            continue
    ...
    lower_content = extract_lower_content(soup)
    if lower_content:
        sections.append({
...
def process_subsection(subsec):
    ...
    subsec_header = subsec.find(['h4', 'h5'])
    if subsec_header:
        header_text = subsec_header.get_text(strip=True)
        content.append(f"{header_text}")  # Add subsection header
    ...
    if subsec_content_div:
        ...
        if text and not is_repealed_section(text):
            content.append(text)
    
    return "\n".join(filter(None, content))

def extract_lower_content(soup):
    """Extract lower part of the page content"""
    lower_content = []
